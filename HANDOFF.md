...
- Race mode hides the solo-oriented blue four-stat bar and uses the info bar for compact race status: racer count, source counts, last update age, and stale/conflict counts. Race action labels switch between `Fit race`/`All racers` and `Fit followed`/`Followed racers` depending on followed racers.
