...
- In sheet race mode, each racer can have multiple Garmin/SPOT/Flymaster source columns. The app refreshes all supported sources and chooses the newest non-stale position per racer (stale thresholds: Flymaster 5 min, Garmin 15 min, SPOT 30 min; if all are stale, newest stale wins). Flymaster group data is fetched once per race and distributed to source rows by `FlymasterId`. Racer popups show per-source diagnostics and warn when fresh sources disagree by more than 1 km. For `/race/transcapixaba-2026`, Garmin MapShare sources backfill KML history for the configured race start/end and cache source history in `localStorage` under `garminRaceTracker.sourceHistory.*` to avoid full history refetches after page refresh. SPOT sources are throttled client-side to ~2.5 minutes per feed.
