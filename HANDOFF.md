...
- In sheet race mode, each racer can have multiple Garmin/SPOT source columns. The app refreshes all supported sources and uses the newest valid position per racer. For `/race/transcapixaba-2026`, Garmin MapShare sources backfill KML history for 2026-07-12 through 2026-07-25 and cache source history in `localStorage` under `garminRaceTracker.sourceHistory.*` to avoid full history refetches after page refresh. SPOT sources are throttled client-side to ~2.5 minutes per feed. If one racer has `UseMapFeatures=true`/`yes`/`1`/`x`, that racer's first Garmin source is used for Garmin routes/waypoints/history map features.
