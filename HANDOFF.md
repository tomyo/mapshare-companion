...
- The top-right `⋮` tracker menu currently has `Enter/Exit race`, `Import KML`, conditional `Hide/Show Garmin features`/`Hide/Show race task`, and `Change racer`; Change racer clears the saved `localStorage` racer and navigates back to `/`. Enter race opens `/race/transcapixaba-2026`; Exit race returns to the saved solo racer or `/`.
- Race mode hides the solo-oriented blue four-stat bar and uses the info bar for compact race status: racer count, source counts, last update age, and stale/conflict counts. Race action labels switch between `Fit race`/`All racers` and `Fit followed`/`Followed racers` depending on followed racers. The race action row includes a `List` button that opens a bottom-sheet racer list with follow checkboxes, Follow all/Clear followed actions, A→Z/proximity sorting, per-racer status, and Map buttons. The bottom sheet supports swipe-down-to-close and uses overscroll containment to avoid mobile pull-to-refresh while interacting with the list.
...
- In sheet race mode, each racer can have multiple Garmin/SPOT/Flymaster source columns. The app refreshes all supported sources and chooses the newest non-stale position per racer (stale thresholds: Flymaster 5 min, Garmin 15 min, SPOT 30 min; if all are stale, newest stale wins). Flymaster group data is fetched once per race and distributed to source rows by `FlymasterId`. Racer popups and marker badges show stale/conflict diagnostics; fresh sources disagreeing by more than 1 km are flagged as source conflicts. For `/race/transcapixaba-2026`, Garmin MapShare sources backfill KML history for the configured race start/end and cache source history in `localStorage` under `garminRaceTracker.sourceHistory.*` to avoid full history refetches after page refresh. SPOT sources are throttled client-side to ~2.5 minutes per feed.
